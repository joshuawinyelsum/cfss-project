...
async def test_admin_survey_visibility_and_idempotency(db, client, student_with_community, admin_token):
    ...
    # We must refresh the object to prevent MissingGreenlet due to expire_on_commit
    db.add(student)
    ...
    for s in surveys:
        if s["id"] == entity_id:
            found = True
            assert s["student_id"] == expected_student_id
            assert s["student_name"] == expected_student_name
            break
