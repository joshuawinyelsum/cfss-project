...
for page in pages:
    ...
    start_idx = content.find('<DashboardLayout>')
    if start_idx != -1:
        # Find the inner div
        inner_div_idx = content.find('<div className="', start_idx)
        if inner_div_idx != -1:
            end_tag_idx = content.find('>', inner_div_idx)
            if end_tag_idx != -1:
                content = content[:end_tag_idx+1] + "\n" + back_btn_jsx + content[end_tag_idx+1:]
